...
public final class JMXDataCollectionConfigFactory {
    ...
    /**
     * <p>
     * Constructor for JMXDataCollectionConfigFactory.</p>
     *
    ...
    /**
     * This method returns the list of MIB objects associated with a
     * particular
    ...
    public Map<String, List<Attrib>> getAttributeMap(String cName, String aSysoid, String anAddress) {
        ...
        for (Mbean mbean : beans.getMbean()) {
            ...
            for (CompAttrib compAttribute : compAttributes) {
                CompMember[] compMembers = compAttribute.getCompMember();
                for (CompMember compMember : compMembers) {
                    Attrib compAttrib = new Attrib();
                    compAttrib.setName(compAttribute.getName() + "|" + compMember.getName());
    ...
    public Map<String, BeanInfo> getMBeanInfo(final String cName) {
        ...
        if (collection == null) {
            ...
            for (Mbean mbean : collection.getMbeans().getMbean()) {
                ...
                beanInfo.setExcludes(Arrays.asList(StringUtils.defaultString(mbean.getExclude()).split(",")));

                List<String> attribNameList = new ArrayList<String>();
                Map<String, List<String>> compAttribNameList = new HashMap<String, List<String>>();

                for (Attrib a : mbean.getAttrib()) {
                    attribNameList.add(a.getName());
                }

                for (CompAttrib ca : mbean.getCompAttrib()) {
                    List<String> caList = new ArrayList<String>();
                    for (CompMember cm : ca.getCompMember()) {
                        caList.add(cm.getName());
                    }
                    compAttribNameList.put(ca.getName(), caList);
                }
     ...
     * creating corresponding MibObject objects and adding them to the
     * supplied
     * MibObject list.
     * <p>
     * @param objectList
